...
import {genericUIDLoop, getCurrentUnixTimestamp, getUser, kill, parseBumpData, parseDeleteData, parseForecastData, parseMediaLink, parseUserlimit} from './utils.js';
...
CHAT_HANDLERS.set('management', async (bot, username, msg, fromIrc) => {
  ...
  if (!(await bot.checkPermission(username, 3, 'G'))) {
    bot.sendChatMsg(
...
CHAT_HANDLERS.set('rngkick', async (bot, username, msg, fromIrc) => {
  ...
  const maxLengthMins = 90;
  const minLengthMins = 5;

  const lengthMins = Math.round(
      (Math.random() * (maxLengthMins - minLengthMins)) + minLengthMins);

  let /** @type {string} */ emote;
  if (lengthMins > (maxLengthMins * .75)) {
    emote = 'pepeLaugh Boot';
  } else if (lengthMins < (maxLengthMins * .25)) {
    emote = 'looks like they got off easy forsenCD';
  } else {
    emote = ':wave: LULE';
  }
  ...
  bot.sendChatMsg(`RNG kicking ${targetUser} for ${lengthDesc}, ${emote}`);
...
CHAT_HANDLERS.set('roulette', async (bot, username, msg, fromIrc) => {
  ...
  const win =
      Math.round(Math.random() * 100) > (100 - bot.rouletteWinPercentage);
...
CHAT_HANDLERS.set('timedkick', async (bot, username, msg, fromIrc) => {
  if (!msg || fromIrc) {
    return;
  }
  if (!(await bot.checkPermission(username, 2, 'I'))) {
    bot.sendChatMsg(
        `${username} does not have permission to timedkick. FeelsWeirdMan`);
    return;
  }
  ...
  if (isNaN(lengthMins) || lengthMins < 0) {
    bot.sendChatMsg(`Failed to parse kick length (found ${msg.split(' ')[1]})`);
    return;
  }
  const lengthSecs = lengthMins * 60;

  const lengthDesc = humanizeDuration(lengthSecs * 1000);
  bot.sendChatMsg(`Kicking ${targetUser} for ${lengthDesc} MODS Boot`);
  bot.sendChatMsg(`/kick ${targetUser}`, true);
...
CHAT_HANDLERS.set('unkick', async (bot, username, msg, fromIrc) => {
  if (!msg || fromIrc) {
    return;
  }
  if (!(await bot.checkPermission(username, 2, 'I'))) {
    bot.sendChatMsg(
        `${username} does not have permission to unkick. FeelsWeirdMan`);
    return;
  }
  ...
  bot.sendChatMsg(
      `${targetUser} has been unkicked and may join again. FeelsOkayMan`);
});
...
CHAT_HANDLERS.set('update', async (bot, username, msg, fromIrc) => {
  ...
  if (!(await bot.checkPermission(username, 5, 'K'))) {
    bot.sendChatMsg(
